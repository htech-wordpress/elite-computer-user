...
const fetchDirectFromFirestore = async (projectId: string): Promise<WebsiteData> => {
    ...
    return {
        homepage: {
            ...
            heroImage: data.homepage?.heroImage || '', // New single hero image
            ...
            trustBadges: data.homepage?.trustBadges || []
        },
        // Map services.items to services array
        services: (data.services?.items || []).map((service: any) => ({
            ...service,
            shortDesc: service.shortDesc || service.description?.substring(0, 100) || '' // Fallback to truncated description
        })),
        about: data.about || {},
        contact: {
            ...data.contact,
